...
make_table(
    ['测试方法', '覆盖范围', '用例数', '通过', '失败'],
    [
        ['自动化测试', '全部8个测试文件', '183', pass_tag(), fail_tag()],
        ['白盒测试', '数据采集/清洗/管理/特征工程/XGBoost', '94', pass_tag(), fail_tag()],
        ['黑盒测试', 'API集成端点', '31', pass_tag(), fail_tag()],
        ['功能测试', '全部6大功能模块', '183', pass_tag(), fail_tag()],
        ['接口测试', '43个API端点', '31', pass_tag(), fail_tag()],
        ['性能测试', 'API响应时间/模型训练时间', '专项', '✅ 达标', '—'],
        ['安全测试', 'JWT认证/RBAC权限/password哈希', '专项', '✅ 通过', '—'],
        ['兼容性测试', 'Python 3.13/pytest 8.4/httpx 0.28', '专项', '✅ 兼容', '—'],
        ['回归测试', '前后2次全量运行', '366', pass_tag(), fail_tag()],
    ],
    [3, 5, 2, 2, 2]
)
...
output_path = os.path.join(os.path.dirname(__file__), '水质监测与预测系统_测试报告.docx')
