...
make_table(
    ...
    [
        ['1', '后端开发', '负责人/后端开发', 'ML模型开发（XGBoost预测器+特征工程+训练脚本）、预测API', '后台管理模块（JWT认证、RBAC权限）'],
        ['2', '姜玉琦', '数据工程师', '数据清洗优化、集成测试', '数据导出功能（Excel/PDF报表）'],
        ['3', '前端开发', '前端开发', 'Vue 3 + Element Plus前端搭建、ECharts可视化图表、3个核心页面', '前端告警页面、管理后台页面'],
        ['4', '文档统筹', '文档/统筹', 'AI Plan文档、第2周进度报告、PPT制作', '告警模块开发、系统集成'],
    ],
    [1.5, 2.5, 3, 5.5, 5]
)
...
make_table(
    ...
    [
        ['T3.1', '异常告警模块（阈值配置、自动触发、历史记录）', '文档统筹', '告警引擎、告警历史API', '2天'],
        ['T3.2', '数据导出功能（Excel/PDF报表）', '数据工程', '导出API、统计报表', '1.5天'],
        ['T3.3', '后台管理（JWT认证、RBAC权限、站点管理）', '后端开发', '登录注册、权限控制、点位管理', '2天'],
...
body('第3周将聚焦功能完善：文档统筹负责异常告警模块（基于GB 3838-2002标准）、姜玉琦负责数据导出功能、后端开发负责后台管理JWT认证和RBAC权限，全员集成联调，为第4周测试验收做准备。')
