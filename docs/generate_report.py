...
make_table(
    ...
    [
        ['1', '谢坤', '负责人/后端开发', '数据清洗模块开发、API服务搭建', 'ML模型开发、预测API'],
        ['2', '姜玉琦', '数据工程师', '数据采集模块（CSV/传感器/手动录入）', '数据导出功能'],
        ['3', '苏航', '前端开发', '项目骨架搭建、单元测试编写', 'Vue 3前端框架+可视化'],
        ['4', '赵宏斌', '文档/统筹', 'AI Plan文档、进度管理、PPT制作', '告警模块开发、系统集成'],
    ],
    [1.5, 2.5, 3, 5, 5]
)
...
make_table(
    ...
    [
        ['T2.1', 'XGBoost水质预测模型训练脚本开发', '谢坤', '训练脚本、模型文件(.pkl)、验证报告', '2天'],
        ['T2.2', '特征工程（相关性分析、特征筛选）', '谢坤', '特征重要性分析报告', '1天'],
        ['T2.3', 'Vue 3 + Element Plus前端项目初始化', '苏航', '前端项目骨架、导航页面', '1.5天'],
        ['T2.4', '水质趋势可视化（Plotly折线图、ECharts仪表盘）', '苏航', '图表组件库', '1.5天'],
        ['T2.5', '预测API接口开发与前后端联调', '谢坤+苏航', '联调通过的预测服务', '1天'],
        ['T2.6', '实时数据展示页面开发', '苏航', '实时数据Web页面', '1天'],
    ],
    [1.5, 6, 2.5, 3.5, 1.5]
)
...
h1('十一、下周展望')
body('第1周已经完成了系统最基础的数据层建设——数据采集和清洗功能全部就绪，API服务运行正常。第2周将聚焦核心智能能力：谢坤负责XGBoost水质预测模型的开发与训练，苏航搭建Vue 3前端实现可视化展示，届时系统将具备完整的“数据采集→清洗→预测→展示”核心链路。')
